...
This repository contains a PHP *Calendar* class. One of its methods takes a year and month as arguments and returns an array of days for the month. The array includes the last days of the previous month and the first days of of the next month to make up full weeks. The screenshot below shows such an array which has been transformed by other PHP and JavaScript code in this repository to create a calendar.

The repository also contains an *Events* class for scheduling events. Events can be created and saved in a database. They can be retrieved and listed in order of when they are scheduled to start. Events can also be deleted. A method called *Events::populateMonth($db, $month)* takes a database object and an array generated by the *Calendar* class as arguments. It returns a modified array which now has events from the database in it associated with the days the events occur on. This modified array can be converted into the calendar shown in the screenshot below using other PHP and JavaScript code in this repository.

The *Events* and *Calendar* classes can be used together to generate arrays of days which include event data in them.

The *virtualcrossing* API has been integrated with the calendar so that the weather forecast for the next 15 days can be seen for today's date. This does not involve the *Calendar* and *Events* classes. It is integrated using JavaScript.

## Screenshot of Calendar

Visiting index.php in a browser renders the current month with the current day shaded grey. Days with an event scheduled are in red. Clicking on a day shows the events for the day under the calendar. Clicking on today's day also generates a weather forecast under the events. Other months can be browsed using the left and right arrows at the top. Clicking on the icon of a user at top right opens a page where you can log in and do other user related actions. The user authentication functionality is provided by code in a separate repository.
